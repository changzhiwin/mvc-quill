...
import org.example.dao.QuestionAnswerDao
import org.example.entities._

class QuizService(dao: QuestionAnswerDao, categoryService: CategoryService)(implicit cs: ContextShift[IO]) {
  ...
  def score(givenAnswers: List[GivenAnswerEntity]): IO[ScoreEntity] = {
    ...
    IO.fromFuture(
      IO(dao.getCorrectQuestionAnswers(questionIds))
    ).map { correctAnswers =>
      val goodAnswers = givenAnswers.filter { answer =>
        correctAnswers.exists {
          case (q, a)  => q == answer.questionId && a == answer.answerId
        }
      }
  ...
  def generate(categoryId: Long): IO[Option[QuizEntity]] =
    categoryService.get(categoryId).flatMap {
      case Some(category) =>
        pickQuestions(category, numberOfQuestions).map(qs => Some(QuizEntity(qs)))

      case None => IO(None)
    }
  
  private def pickQuestions(category: CategoryEntity, n: Int): IO[List[QuestionEntity]] = {
    val randomQAs = IO.fromFuture(IO(
      dao.pickByCategoryId(category.id, n)
    ))

    randomQAs.map { qas =>
      qas.map {
        case (q, as) => QuestionEntity.fromRecord(q, as)
      }.toList
    }
  }
}